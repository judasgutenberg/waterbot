...
<html>
   <head>
	  ...
	  <script>
	     function moveFunction(command) {
		var xmlhttp = new XMLHttpRequest();
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			}
		};
		let url = "propulsion.php?command=" + encodeURI(command);
		console.log(url);
...
?>
	  ...
	  <button onclick='moveFunction("left")' type='button'>left</button>
	  <button onclick='moveFunction("right")' type='button'>right</button><br>
	  <button onclick='moveFunction("go")' type='button'>go</button>
	  <button onclick='moveFunction("rev")' type='button'>reverse</button>
	  <button onclick='moveFunction("kill")' type='button'>DIE!</button>
         <?php pan_controls(); ?>
         <?php user_buttons(); ?>
